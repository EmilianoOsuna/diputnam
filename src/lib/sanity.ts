...
// Build-time content client. Production reads the published perspective of the CMS
// through its CDN; SANITY_LOCAL_DATASET (a JSON array of documents, see
// studio/scripts/seed-local.ts) evaluates the same GROQ locally with groq-js so the
// site can be built and tested without a project. Neither set → fail loudly.
