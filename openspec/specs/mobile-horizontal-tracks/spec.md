...
- **WHEN** `/eredita/eredita-art/` se abre a 390 px y se desplaza hasta Tipologías
- **THEN** se ve el encabezado (kicker, título, intro, contador, barra) y debajo la pista con las tipologías del proyecto
...
En viewports de 900 px o más, ambas secciones SHALL conservar el comportamiento actual (pin y avance controlado por scroll). La pista móvil SHALL ser navegable con teclado (foco en cada tarjeta y desplazamiento con flechas), anunciar la posición actual a tecnologías de asistencia y respetar `prefers-reduced-motion` desactivando el desplazamiento suave pero manteniendo el swipe. Las flechas de teclado con el foco dentro de la tira de miniaturas SHALL mover la selección de la tira, no la pista.

#### Scenario: Desktop sin regresión
- **WHEN** `/eredita/eredita-art/` o `/unete/` se abre a 1440 px
- **THEN** Tipologías y Nuestra cultura se comportan como antes del cambio, y en Tipologías el avance por rueda o trackpad sigue siendo de un panel por gesto aunque el puntero esté sobre la tira de miniaturas
