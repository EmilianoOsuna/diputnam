...
2. **Reposo en porcentajes**: `slideTo()` deja de escribir px y escribe `wrapper.style.translate = '0 ' + (-index * 100 / panels.length) + '%'` (el porcentaje del `translate` es relativo al cajón del propio stage, que mide `N × 100 %` del contenedor). Las capas de parallax en reposo reciben `progress ∈ {-1, 0, 1}`: `.panel-media` → `0 ±60%` (su alto es el del panel, así que el porcentaje equivale a los px de hoy), el título → `0 ∓60px`, y `.panel-content` → `0` en el panel activo y `0 ±100%` en los vecinos (su propio alto, no el del panel: es inexacto, pero esos paneles quedan fuera de pantalla y recortados por `overflow: hidden`, y el siguiente arrastre vuelve a px desde la posición congelada); sólo durante el arrastre recibe px. Con esto, si el contenedor cambia de alto sin que corra JS, el layout recoloca el stage y el panel activo sigue cubriendo el viewport.
