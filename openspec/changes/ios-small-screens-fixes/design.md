...
2b. **Capa fija para el degradado** (18 sep, segunda prueba en iPhone): con la foto en `z-index: -2` animándose debajo del degradado `.panel-shade` (sin capa propia, `z-index: -1`), Safari promovía el degradado a capa sólo durante la transición y lo devolvía al aterrizar: el velo entre foto y título desaparecía durante el sweep y reaparecía de golpe. En el bloque `.is-slider` el orden pasa a positivo (`media 0`, `shade 1`, `content 2`) y el degradado lleva `will-change: transform; transform: translateZ(0)` para tener capa siempre.
